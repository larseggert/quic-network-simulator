...
// from https://github.com/zmb3/hexdump with modifications

template <typename T, T RowSize, bool ShowAscii> class CustomHexdumpBase {
public:
...
class CustomHexdump
    : public CustomHexdumpBase<std::size_t, RowSize, ShowAscii> {
public:
  CustomHexdump(unsigned char *data, std::size_t length)
      : CustomHexdumpBase<std::size_t, RowSize, ShowAscii>(data, length) {}
};

template <typename T, T RowSize, bool ShowAscii>
std::ostream &operator<<(std::ostream &out,
                         const CustomHexdumpBase<T, RowSize, ShowAscii> &dump) {
